                ...
                <section className={styles.analysisSection}>
                    ...
                    <div className={styles.digitGrid}>
                        {digitAnalysis.map((percentage, digit) => {
                            // Calculate Rank
                            const indexed = digitAnalysis.map((p, i) => ({ digit: i, percentage: p }));
                            // Sort descending (higher % first)
                            indexed.sort((a, b) => b.percentage - a.percentage);

                            const rank = indexed.findIndex(item => item.digit === digit);
                            const total = 10;

                            let strokeColor = "#374151"; // Default Grey

                            if (rank === 0) strokeColor = "#10b981";       // 1st: Green
                            else if (rank === 1) strokeColor = "#3b82f6";  // 2nd: Blue
                            else if (rank === total - 1) strokeColor = "#ef4444"; // Last: Red
                            else if (rank === total - 2) strokeColor = "#d4af37"; // 2nd Last: Gold/Yellow

                            // Keep fill logic
                            const fillPercentage = maxPercentage > 0 ? (percentage / maxPercentage) * 100 : 0

                            // Keep old isLowest/Highest for text styling if needed, or remove if strictly rank-based styling desired.
                            // For safety, we keep them for the classNames below but override the RING color above.
                                ...
                                <div key={digit} className={styles.digitContainer}>
                                    ...
                                    <div className={styles.digitCircleWrapper}>
                                        <div className={styles.digitCircle}>
                                            ...
                                            <div
                                                className={`${styles.digitNumber} ${digit === currentDigit
                                                    ? styles.current
                                                    : isLowest
                                                        ? styles.lowest
                                                        : isHighest
                            ...
                            <div className={styles.digitsContent}>
                                <div className={styles.digitsGrid}>
                                    {lastDisplayedDigits.map((digit, index) => (
                                        <div
                                            key={index}
                                            className={`${styles.digitItem} ${digit % 2 === 0 ? styles.evenDigit : styles.oddDigit} ${index === lastDisplayedDigits.length - 1 ? styles.latest : ""
                                                }`}
